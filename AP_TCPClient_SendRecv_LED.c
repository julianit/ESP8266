/*
	...
	1. Connects to access point (AP)
	2. When connected prints the IP Address
	3. Creates TCP client and tries to connect to TCP server on port 5555
	4. When connects to TCP server send data.
	5. Then wait until we send data back.
	6. If you connect light-emitting diod on GP05 it be ON if you send 1 and OFF if send something else. 
		see the code line:	int v = (pData[0] == '1');
								GPIO_OUTPUT_SET(LED_GPIO, v);
								
	TODO: 
		1. Clead the code
		2. Add schema how the LED is connect
*/
...
//static void ICACHE_FLASH_ATTR espconn_regist_recvcb()
//{
//	recvCB(arg, pData, len);
//}
...
{
    // Set AP settings
    char ssid[32] = WIFI_CLIENTSSID;			//"AP name ";
    char password[64] = WIFI_CLIENTPASSWORD;	//"AP password";
